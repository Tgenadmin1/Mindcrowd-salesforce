({
    recorData: function(component, event,helper,userContactId,gameId,questionNumber,userInputData,correctAnswer,isCorrect,reactionTime,isPracticeQuestion,participantGameInfoId,firstTime,isIgnore,inputkeyPress,indivisualInputTime,totalTimeForUserResponse, firstResponce, imageNames, round){
        //console.log("Sunil:Input Results helper",userContactId,gameId," questionNumber: ",questionNumber,userInputData,correctAnswer,isCorrect,reactionTime,isPracticeQuestion,participantGameInfoId,firstTime,isIgnore,inputkeyPress);
         let data = {
            ...
            First_Input_Time__c:firstTime,
            Don_t_Know__c:isIgnore,
            Participant_First_Response__c: inputkeyPress,
            Time_for_Each_Box__c:indivisualInputTime,
            Total_Time_for_All_Boxes__c:totalTimeForUserResponse,
            Order_of_Answers__c:firstResponce,
            ImageName__c:imageNames,
            Round__c:round
        };
      
        //console.log('helper-----Sunil:data is : ',data);
        var action = component.get("c.saveGameResponse");
        action.setParams({"sobj":JSON.stringify(data)}); 
        var promise= new Promise(function(resolve, reject) { 
            //console.log("Sunil: inside the promise fun:");
        action.setCallback(this,function(a) {            
            let state = a.getState();
            //console.log('Sunil:in callback record data value---------',state);  
            if (a.getState() === 'SUCCESS') {
                resolve(a.getReturnValue());
                let name = a.getReturnValue();
                //console.log('sunil:incallback success record data value----------',name);            
            }
            else if(state==="ERROR"){
                reject(new Error(a.getError()));
                let message='';
                let errors = a.getError();
                //console.log('sunil:incallback error record data value=======',errors);
        ...
        //console.log('Sunil:ENQUE ACTION -> 1');
        // $A.enqueueAction(action);  
        $A.getCallback(function() {
            $A.enqueueAction(action);
        })();
      });
      //console.log("Sunil: after the promise fun:",promise);
    },
   ...
   participantGameInfo: function(component,event,helper,contactId,language,gameId,startDateTime,gamePlayStatus,ipAddress,browserName,device,screenResolution) {
    ...
    gameNameInParticipantGameInfo: function(component,event,helper,contactId,gameId,participantGameInfoId,ipAddress,browserName,device) {
        ...
        //console.log('face name data',data);
    ...
    participantGameInfoUpdate: function(component,event,helper,contactId,language,gameId,endDateTime,gamePlayStatus,participantGameInfoId,screenResolution) {
        ...
        try{
            //console.log('Sunil:ENQUE ACTION -> 3');
            // $A.enqueueAction(action);
            $A.getCallback(function() {
                $A.enqueueAction(action);
            })();
    ...
    recorDataTowerOfGame: function(component, event, helper,reactionTime,userInputData,questionNumber,userContactId, gameId,participantGameInfoId,isPracticeQuestion,stackTrialTillFirstMove, round){
        //console.log("Input Results helper",reactionTime,userInputData,questionNumber,userContactId, gameId,participantGameInfoId,isPracticeQuestion);
        let data = {
            Contact_Name__c:userContactId,
            Game_Name__c:gameId,
            Question_Number__c:questionNumber,
            ...
            Participant_Game_InfoID__c:participantGameInfoId,
            Is_Practice_Question__c:isPracticeQuestion,
            First_Input_Time__c:stackTrialTillFirstMove,
            Round__c:round
        };
        //console.log('data is : ',data);
        var action = component.get("c.saveGameResponse");
        action.setParams({"sobj":JSON.stringify(data)}); 
       // //console.log('userContactId: '+userContactId+'gameid: '+gameId+'question: ' + questionNumber +' responce: '+ userInputData +' correct: '+ isCorrect +'time: '+reactionTime+'isPracticeQuestion:'+isPracticeQuestion);
        action.setCallback(this,function(a) {           
            var state = a.getState();
            //console.log('record data state value---------',state);  
            if (state === "SUCCESS") {
                var name = a.getReturnValue();
                //console.log('record data name value----------',name);            
            }
            else if(state==="ERROR"){
                let message='';
                let errors = a.getError();
        ...
        //console.log('Sunil:ENQUE ACTION -> 2');
        // $A.enqueueAction(action);  
        $A.getCallback(function() {
            $A.enqueueAction(action);
        })();
    ...
    participantGameInfoUpdateTotalTimeRoundOne: function(component,event,helper,userContactId,gameId,participantGameInfoId,totalTimeForRound,currentScreent) {
        ...
        if(currentScreent=='27'){
            ...
            data = {
                ...
                Game_Name__c:gameId,
                Id:participantGameInfoId,
                Total_Time_for_STACKS__c:totalTimeForRound
            };
        }
        else if(currentScreent=='118'){
            //console.log('i am in 243 screen end game');
            data = {
                Contact_Name__c:userContactId,
                Game_Name__c:gameId,
